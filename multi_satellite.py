...
class GroundStation:
    ...
    def __init__(self, name, latitude, longitude):
        ...
        self.x = EARTH_RADIUS * np.cos(self.latitude) * np.cos(self.longitude)

        self.y = EARTH_RADIUS * np.cos(self.latitude) * np.sin(self.longitude)

        self.z = EARTH_RADIUS * np.sin(self.latitude)
...
def update(frame):
    ...
    for sat in satellites:
        ...
        coverage_angle = np.arccos(
            EARTH_RADIUS /
            (EARTH_RADIUS + sat.altitude)
            )
        
        coverage_radius = (
            EARTH_RADIUS *
            np.sin(coverage_angle)
            )
        ...
        circle_x = (
            ground_x +
            coverage_radius*np.cos(phi)
        )

        circle_y = (
            ground_y +
            coverage_radius*np.sin(phi)
        )
        ...
        sat.coverage.set_data(
            circle_x,
            circle_y
        )

        sat.coverage.set_3d_properties(
            circle_z
        )
